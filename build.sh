...
# Tomatime CMake Build Script for WSL with MinGW-32
# This script builds the Tomatime project using CMake and MinGW-32 cross-compiler
#
# Usage:
...
show_help() {
    echo -e "${CYAN}Tomatime Build Script${NC}"
    echo ""
...
if [ -f "tomatime.exe" ]; then
    ...
    if [ "$(realpath "$OUTPUT_DIR")" != "$(realpath "../$BUILD_DIR")" ]; then
        mkdir -p "$OUTPUT_DIR"
        cp "tomatime.exe" "$OUTPUT_DIR/"
        echo -e "${CYAN}Output: $OUTPUT_DIR/tomatime.exe${NC}"
    else
        echo -e "${CYAN}Output: $(pwd)/tomatime.exe${NC}"
    fi
