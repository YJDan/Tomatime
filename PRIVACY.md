...
**Tomatime** is a desktop application focused on personal time management. We highly value your privacy, and this policy explains how we handle your data.

## Data Collection and Usage

### Information We Do **Not** Collect

Tomatime explicitly commits to:
...
Tomatime only saves the following information locally in `%LOCALAPPDATA%\Tomatime\`:

1. **Configuration Data**
   Includes interface settings, timer configurations, and user preferences, stored in `config.ini`.

2. **Log Information**
   Saved in `Tomatime_Logs.log`, containing:
...
4. **Plugin Data**
   If you use the plugin feature, plugins may read/write data to `resources\plugins\output.txt`. Plugins run locally and Tomatime does **not** transmit any plugin data over the network.

### Network Access Information

Tomatime only accesses the internet in the following circumstances:

* **Update Checking**
  Tomatime automatically checks for updates on startup and also supports manual update checking. The program connects to the GitHub API:
  `https://api.github.com/repos/vladelaina/Catime/releases/latest`
  It only sends basic HTTP requests and the user agent string `"Tomatime Update Checker"`, **without any personal data**.

## Data Protection and Deletion

All data is stored locally, and Tomatime does not perform any remote synchronization or uploads.
To completely remove data, you can manually delete the configuration files and log files.

## Permission Usage Information

System permissions requested by Tomatime are only used to implement core functions:
